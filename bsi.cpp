...
int main (){
    ...
    cin>> selection;
    if (selection == 1 ){
       cout<< calcuate_01_03(0.000001,2500);
    }
    else if(selection==2){
        cout<< calculate_01_04(98,5000);
    }
    else if (selection==3){
        pair<double,double> result;
        result = calculate_01_05(25,30);
        cout<<"Failure Rate : "<<result.first<<" Reliability : "<<result.second;
    }
    else if (selection==4){
         cout<<calculate_01_06(1000,500);
    }
    else if (selection==5){

vector<int> population = {17,25,32,10,6,5,8,9,12,17,46,64,83,70,10,15,2,8,29,11};
cout<<calculate36(population);
    }
return 0;
}